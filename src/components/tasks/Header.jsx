"use client";
import React from "react";

const Header = ({ onNavigate }) => {
  return (
    <header className="w-full h-20 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
      <div className="flex justify-between items-center px-0 py-5 mx-auto my-0 max-w-screen-xl h-20 max-sm:px-4 max-sm:py-5">
        <div className="flex gap-3 items-center">
          <div>
            <svg
              width="38"
              height="30"
              viewBox="0 0 38 30"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="logo-icon"
              style={{ width: "37.5px", height: "30px" }}
            >
              <g clipPath="url(#clip0_5_2298)">
                <path
                  d="M18.75 0C19.7871 0 20.625 0.837891 20.625 1.875V5.625H27.6562C29.9883 5.625 31.875 7.51172 31.875 9.84375V25.7812C31.875 28.1133 29.9883 30 27.6562 30H9.84375C7.51172 30 5.625 28.1133 5.625 25.7812V9.84375C5.625 7.51172 7.51172 5.625 9.84375 5.625H16.875V1.875C16.875 0.837891 17.7129 0 18.75 0ZM12.1875 22.5C11.6719 22.5 11.25 22.9219 11.25 23.4375C11.25 23.9531 11.6719 24.375 12.1875 24.375H14.0625C14.5781 24.375 15 23.9531 15 23.4375C15 22.9219 14.5781 22.5 14.0625 22.5H12.1875ZM17.8125 22.5C17.2969 22.5 16.875 22.9219 16.875 23.4375C16.875 23.9531 17.2969 24.375 17.8125 24.375H19.6875C20.2031 24.375 20.625 23.9531 20.625 23.4375C20.625 22.9219 20.2031 22.5 19.6875 22.5H17.8125ZM23.4375 22.5C22.9219 22.5 22.5 22.9219 22.5 23.4375C22.5 23.9531 22.9219 24.375 23.4375 24.375H25.3125C25.8281 24.375 26.25 23.9531 26.25 23.4375C26.25 22.9219 25.8281 22.5 25.3125 22.5H23.4375ZM15.4688 15C15.4688 14.3784 15.2218 13.7823 14.7823 13.3427C14.3427 12.9032 13.7466 12.6562 13.125 12.6562C12.5034 12.6562 11.9073 12.9032 11.4677 13.3427C11.0282 13.7823 10.7812 14.3784 10.7812 15C10.7812 15.6216 11.0282 16.2177 11.4677 16.6573C11.9073 17.0968 12.5034 17.3438 13.125 17.3438C13.7466 17.3438 14.3427 17.0968 14.7823 16.6573C15.2218 16.2177 15.4688 15.6216 15.4688 15ZM24.375 17.3438C24.9966 17.3438 25.5927 17.0968 26.0323 16.6573C26.4718 16.2177 26.7188 15.6216 26.7188 15C26.7188 14.3784 26.4718 13.7823 26.0323 13.3427C25.5927 12.9032 24.9966 12.6562 24.375 12.6562C23.7534 12.6562 23.1573 12.9032 22.7177 13.3427C22.2782 13.7823 22.0312 14.3784 22.0312 15C22.0312 15.6216 22.2782 16.2177 22.7177 16.6573C23.1573 17.0968 23.7534 17.3438 24.375 17.3438ZM2.8125 13.125H3.75V24.375H2.8125C1.25977 24.375 0 23.1152 0 21.5625V15.9375C0 14.3848 1.25977 13.125 2.8125 13.125ZM34.6875 13.125C36.2402 13.125 37.5 14.3848 37.5 15.9375V21.5625C37.5 23.1152 36.2402 24.375 34.6875 24.375H33.75V13.125H34.6875Z"
                  fill="#4F46E5"
                />
              </g>
              <defs>
                <clipPath id="clip0_5_2298">
                  <path d="M0 0H37.5V30H0V0Z" fill="white" />
                </clipPath>
              </defs>
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-indigo-900 max-sm:text-xl">
            AI Task Buddy
          </h1>
        </div>
        <div className="flex gap-6 items-center max-sm:gap-4">
          <button 
            onClick={() => onNavigate && onNavigate('dashboard')}
            className="text-indigo-600 hover:text-indigo-800"
          >
            Dashboard
          </button>
          <button className="w-5 h-5 cursor-pointer" aria-label="Help">
            <svg
              width="21"
              height="20"
              viewBox="0 0 21 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="help-icon"
              style={{ width: "20px", height: "20px" }}
            >
              <g clipPath="url(#clip0_5_2313)">
                <path
                  d="M18.625 10C18.625 7.84512 17.769 5.77849 16.2452 4.25476C14.7215 2.73102 12.6549 1.875 10.5 1.875C8.34512 1.875 6.27849 2.73102 4.75476 4.25476C3.23102 5.77849 2.375 7.84512 2.375 10C2.375 12.1549 3.23102 14.2215 4.75476 15.7452C6.27849 17.269 8.34512 18.125 10.5 18.125C12.6549 18.125 14.7215 17.269 16.2452 15.7452C17.769 14.2215 18.625 12.1549 18.625 10ZM0.5 10C0.5 7.34784 1.55357 4.8043 3.42893 2.92893C5.3043 1.05357 7.84784 0 10.5 0C13.1522 0 15.6957 1.05357 17.5711 2.92893C19.4464 4.8043 20.5 7.34784 20.5 10C20.5 12.6522 19.4464 15.1957 17.5711 17.0711C15.6957 18.9464 13.1522 20 10.5 20C7.84784 20 5.3043 18.9464 3.42893 17.0711C1.55357 15.1957 0.5 12.6522 0.5 10ZM7.13281 6.45703C7.44141 5.58594 8.26953 5 9.19531 5H11.4727C12.8359 5 13.9375 6.10547 13.9375 7.46484C13.9375 8.34766 13.4648 9.16406 12.6992 9.60547L11.4375 10.3281C11.4297 10.8359 11.0117 11.25 10.5 11.25C9.98047 11.25 9.5625 10.832 9.5625 10.3125V9.78516C9.5625 9.44922 9.74219 9.14062 10.0352 8.97266L11.7656 7.98047C11.9492 7.875 12.0625 7.67969 12.0625 7.46875C12.0625 7.14062 11.7969 6.87891 11.4727 6.87891H9.19531C9.0625 6.87891 8.94531 6.96094 8.90234 7.08594L8.88672 7.13281C8.71484 7.62109 8.17578 7.875 7.69141 7.70312C7.20703 7.53125 6.94922 6.99219 7.12109 6.50781L7.13672 6.46094L7.13281 6.45703ZM9.25 13.75C9.25 13.4185 9.3817 13.1005 9.61612 12.8661C9.85054 12.6317 10.1685 12.5 10.5 12.5C10.8315 12.5 11.1495 12.6317 11.3839 12.8661C11.6183 13.1005 11.75 13.4185 11.75 13.75C11.75 14.0815 11.6183 14.3995 11.3839 14.6339C11.1495 14.8683 10.8315 15 10.5 15C10.1685 15 9.85054 14.8683 9.61612 14.6339C9.3817 14.3995 9.25 14.0815 9.25 13.75Z"
                  fill="#4B5563"
                />
              </g>
              <defs>
                <clipPath id="clip0_5_2313">
                  <path d="M0.5 0H20.5V20H0.5V0Z" fill="white" />
                </clipPath>
              </defs>
            </svg>
          </button>
          <button className="w-5 h-5 cursor-pointer" aria-label="Notifications">
            <svg
              width="18"
              height="20"
              viewBox="0 0 18 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="notification-icon"
              style={{ width: "20px", height: "20px" }}
            >
              <g clipPath="url(#clip0_5_2316)">
                <path
                  d="M9.2501 0C8.5587 0 8.0001 0.558594 8.0001 1.25V1.94922C5.16807 2.39844 3.0001 4.85156 3.0001 7.8125V9.11719C3.0001 10.8906 2.39463 12.6133 1.28917 13.9961L0.707134 14.7266C0.480572 15.0078 0.437603 15.3945 0.593853 15.7188C0.750103 16.043 1.07823 16.25 1.4376 16.25H17.0626C17.422 16.25 17.7501 16.043 17.9064 15.7188C18.0626 15.3945 18.0196 15.0078 17.7931 14.7266L17.211 14C16.1056 12.6133 15.5001 10.8906 15.5001 9.11719V7.8125C15.5001 4.85156 13.3321 2.39844 10.5001 1.94922V1.25C10.5001 0.558594 9.94151 0 9.2501 0ZM9.2501 3.75H9.5626C11.8048 3.75 13.6251 5.57031 13.6251 7.8125V9.11719C13.6251 10.9883 14.1681 12.8125 15.1759 14.375H3.32432C4.33213 12.8125 4.8751 10.9883 4.8751 9.11719V7.8125C4.8751 5.57031 6.69542 3.75 8.9376 3.75H9.2501ZM11.7501 17.5H9.2501H6.7501C6.7501 18.1641 7.01182 18.8008 7.48057 19.2695C7.94932 19.7383 8.58604 20 9.2501 20C9.91417 20 10.5509 19.7383 11.0196 19.2695C11.4884 18.8008 11.7501 18.1641 11.7501 17.5Z"
                  fill="#4B5563"
                />
              </g>
              <defs>
                <clipPath id="clip0_5_2316">
                  <path d="M0.5 0H18V20H0.5V0Z" fill="white" />
                </clipPath>
              </defs>
            </svg>
          </button>
          <img
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/497bdb00407db5e6221fd8e93487be13a65bc3e4"
            alt="Profile"
            className="w-10 h-10 rounded-full cursor-pointer"
            onClick={() => onNavigate && onNavigate('user')}
          />
        </div>
      </div>
    </header>
  );
};

export default Header;
